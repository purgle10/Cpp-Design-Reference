...
namespace MySTL{
	...
	public:
		...
		binary_search_tree(const binary_search_tree&) = delete;// 担心浅拷贝？二叉搜索树不允许拷贝构造和复制
	...
	namespace Detail{
		...
		class bst_iter :
			public iterator<forward_iterator_tag, typename ::MySTL::binary_search_tree<typename T::value_type>::value_type>{
		private:
			template<class T>
		...
		private:
			const T *ptr_;
			cntrPtr container_;
			stack<const T *> parent_;// 保存从root到ptr_的父节点路径
